"""ATeam alpha plugin overrides used by integration tests."""
...
class AlphaAgentAdapter:
    """Simple sync/async adapter registered by the alpha plugin."""

    def __init__(self):
        """Create the adapter and its raw agent."""
        self._raw = AlphaRawAgent("alpha-raw")
    ...
    def metadata(self) -> dict[str, str]:
        ...
        return {"adapter": "alpha", "mode": "writer"}
...
def activated(plugin, rag2f_instance):
    """Register the alpha agent as the plugin default."""
    key = "a_team_alpha_plugin.writer"
...
def deactivated(plugin, rag2f_instance):
    """Remove the alpha agent during plugin deactivation."""
    rag2f_instance.a_team.unregister("a_team_alpha_plugin.writer")
